...
public class Jooby implements Router, LifeCycle, Registry {
    ...
    public static void run(final Supplier<? extends Jooby> app, final String... args) {
        IntStream.range(0, args.length)
                .filter(it -> "configuration.file".equals(args[it]))
                .findFirst()
                .ifPresent(it -> System.setProperty("configuration.file", args[it + 1]));
    ...
    private Injector bootstrap(final Config args, final Consumer<List<Route.Definition>> rcallback) throws Throwable {
        final String confFile = System.getProperty("configuration.file", "application.conf");
        Config appconf = ConfigFactory.parseResources(confFile);
